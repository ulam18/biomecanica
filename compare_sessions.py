...
Gera uma tabela-resumo no terminal e um grafico comparativo. Prefere
`lateral_dynamic_filtered` (desvio relativo ao baseline neutro) quando TODAS
as sessoes comparadas tem essa coluna; `lateral_absolute` (desvio_lateral_
filtrado) nunca fica escondido -- sempre aparece na tabela. Sessoes antigas
(exportadas antes da lateralidade dinamica existir) nao tem essa coluna: a
comparacao cai para o absoluto para TODAS as sessoes, com aviso explicito no
console (nunca se inventa um valor dinamico para uma sessao que nao o tem).
Nao emite diagnostico.
"""
...
# Coluna opcional (sessoes exportadas antes da lateralidade dinamica existir
# nao a tem); quando presente em TODAS as sessoes comparadas, e a preferida.
DYNAMIC_LATERAL_COLUMN = "lateral_dynamic_filtered"
...
class SessionData:
    ...
    def col_or_nan(self, name: str) -> np.ndarray:
        """Como `col`, mas celulas vazias (frame invalido/nao calibrado) viram NaN, nunca 0."""
        return np.array(
            [float(r[name]) if r.get(name, "") not in ("", None) else np.nan for r in self.rows],
            dtype=float,
        )

    @property
    def has_dynamic(self) -> bool:
        return bool(self.rows) and DYNAMIC_LATERAL_COLUMN in self.rows[0]
...
def session_stats(s: SessionData) -> dict:
    ...
    if s.has_dynamic:
        dyn = s.col_or_nan(DYNAMIC_LATERAL_COLUMN)
        dyn_valid = dyn[~np.isnan(dyn)]
        stats["lateral_dinamico_max_abs"] = (
            float(np.max(np.abs(dyn_valid))) if len(dyn_valid) else None
        )
        stats["lateral_dinamico_medio_abs"] = (
            float(np.mean(np.abs(dyn_valid))) if len(dyn_valid) else None
        )
    else:
        stats["lateral_dinamico_max_abs"] = None
        stats["lateral_dinamico_medio_abs"] = None
...
def print_summary_table(stats_list: list[dict]) -> None:
    cols = [
        ...
        "desvio_lateral_max_abs", "lateral_dinamico_max_abs", "lateral_dinamico_medio_abs",
        "repeticoes", "amplitude_media", "amplitude_cv", "duracao_media_s", "duracao_cv",
    ]
...
def plot_comparison(sessions: list[SessionData], out_path: str, normalize_time: bool) -> str:
    use_dynamic = all(s.has_dynamic for s in sessions)
    if not use_dynamic:
        missing = [s.name for s in sessions if not s.has_dynamic]
        print(
            "[aviso] comparacao lateral usa 'lateral_absolute' (desvio_lateral_filtrado): "
            f"sessao(oes) sem lateral_dynamic (versao antiga ou nao calibrada): {missing}. "
            "Nenhum valor dinamico foi inventado para elas."
        )
    lateral_col = DYNAMIC_LATERAL_COLUMN if use_dynamic else "desvio_lateral_filtrado"
    lateral_label = "Desvio lateral dinamico (filtrado)" if use_dynamic else "Desvio lateral absoluto (filtrado)"
    ...
    for s in sessions:
        ...
        if normalize_time and len(t) and t[-1] > 0:
            t = t / t[-1]
        lateral = s.col_or_nan(lateral_col) if use_dynamic else s.col(lateral_col)
        ax1.plot(t, s.col("abertura_filtrada"), label=s.name, linewidth=1.3)
        ax2.plot(t, lateral, label=s.name, linewidth=1.3)
